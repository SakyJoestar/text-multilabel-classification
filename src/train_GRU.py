...
Entrenamiento de una red GRU (RNN con memoria) para clasificación de sentimiento
sobre el dataset "Twitter US Airline Sentiment Dataset" preprocesado
en preprocessing.py

Guarda resultados en results/GRU/
"""
...
class SentimentGRU(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.bidirectional:
            ...
            h = torch.cat((fwd_last, bwd_last), dim=1)  # [B, 2H]
        else:
            h = hidden[-1, :, :]  # [B, H]
...
def plot_curves(train_losses, val_losses, val_f1s):
    ...
    plt.title("Validation F1 (macro) per Epoch - GRU")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, "f1_curve_gru.png"))
    plt.close()
    ...
    plt.title("Train Loss per Epoch - GRU")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, "loss_curve_gru.png"))
    plt.close()
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Train vs Val Loss - GRU")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, "train_val_loss_curve_gru.png"))
    plt.close()
...
def main():
    ...
    best_val_f1 = 0.0
    best_model_path = os.path.join(RESULTS_DIR, "gru_model.pth")
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        # Guardar mejor modelo según F1-macro
    ...
    with open(metrics_path, "w", encoding="utf-8") as f:
        f.write("Test metrics for GRU sentiment classifier\n\n")
    ...
    # ---------------------- Guardar vocabulario ----------------------
    vocab_path = os.path.join(RESULTS_DIR, "gru_vocab.json")
